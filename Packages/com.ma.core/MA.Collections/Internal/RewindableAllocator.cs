...
// com.unity.collections 2.0+ version of RewindableAllocator
// Allows re-allocating a rewindable allocator

namespace MA.Collections.Internal
{
    struct Spinner
    {
        int m_Lock;

        /// <summary>Continually spin until the lock can be acquired.</summary>
        ...
        /// <summary>Try to acquire the lock and immediately return without spinning.</summary>
    ...
    /// <summary>An allocator that is fast like a linear allocator, is threadsafe, and automatically invalidates
    /// all allocations made from it, when "rewound" by the user.</summary>
    [BurstCompile]
    public struct RewindableAllocator : AllocatorManager.IAllocator
    {
        ...
        /// <summary>Initializes the allocator. Must be called before first use.</summary>
        ...
        /// <summary>Property to get and set enable block free flag, a flag indicating whether the allocator should enable individual block to be freed.</summary>
        ...
        /// <summary>Retrieves the number of memory blocks that the allocator has requested from the system.</summary>
        public int BlocksAllocated => (int)(m_last + 1);

        /// <summary>Retrieves the size of the initial memory block, as requested in the Initialize function.</summary>
        public int InitialSizeInBytes => (int)(m_block[0].m_bytes);

        /// <summary>Retrieves the maximum memory block size.</summary>
        internal long MaxMemoryBlockSize => kMaxMemoryBlockSize;

        /// <summary>Retrieves the total bytes of the memory blocks allocated by this allocator.</summary>
        ...
        /// <summary>Rewind the allocator; invalidate all allocations made from it, and potentially also free memory blocks
        /// it has allocated from the system.</summary>
        public void Rewind()
        {
        ...
        /// <summary>Dispose the allocator. This must be called to free the memory blocks that were allocated from the system.</summary>
        public void Dispose()
        {
            if (JobsUtility.IsExecutingJob)
        ...
        /// <summary>All allocators must implement this property, in order to be installed in the custom allocator table.</summary>
        ...
        /// <summary>Try to allocate, free, or reallocate a block of memory. This is an internal function, and
        /// is not generally called by the user.</summary>
        ...
        /// <summary>Retrieve the AllocatorHandle associated with this allocator. The handle is used as an index into a
        /// global table, for times when a reference to the allocator object isn't available.</summary>
        ...
        /// <summary>Retrieve the Allocator associated with this allocator.</summary>
        /// <value>The Allocator retrieved.</value>
        public Allocator ToAllocator { get { return m_handle.ToAllocator; } }

        /// <summary>Check whether this AllocatorHandle is a custom allocator.</summary>
        ...
        /// <summary>Check whether this allocator will automatically dispose allocations.</summary>
        ...
        /// <summary>Allocate a NativeArray of type T from memory that is guaranteed to remain valid until the end of the
        ...
        {
            return CollectionHelper.CreateNativeArray<T>(length, Handle);
        }

        /// <summary>Allocate a NativeList of type T from memory that is guaranteed to remain valid until the end of the
        ...
        /// up to 1/2 of the total final capacity would be wasted, because the memory can't be dynamically freed.</summary>
