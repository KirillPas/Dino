Shader "Hidden/Flora/DebugOcclusionTest"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "Packages/com.ma.flora/ShaderLibrary/TextureXR.hlsl"
            #include "Packages/com.unity.render-pipelines.core/ShaderLibrary/UnityInstancing.hlsl"
            #include "Packages/com.ma.flora/Runtime/Rendering/Occlusion/OcclusionCullingDebugShaderVariables.cs.hlsl"
            #include "Packages/com.ma.flora/Runtime/Rendering/Occlusion/OcclusionCommon.cs.hlsl"
            ...
            // See: "Packages/com.unity.render-pipelines.core/ShaderLibrary/Debug.hlsl" `OverlayHeatMap`
            float4 OverlayHeapMapColor(float cost, float costScreenTotal, float opacity)
            {
                ...
                int colorIndex = 1 + (int)(cost * colorRangeScale);

                colorIndex = clamp(colorIndex, 1, DEBUG_COLORS_COUNT-1); // Skip colorIndex 0 (black)
                float4 gradient = kDebugColorGradient[colorIndex];
                
                float4 color = float4(PositivePow(gradient.rgb, 2.2), opacity * gradient.a);
                return color;
            }
            ...
            float4 Frag(Varyings input) : SV_Target
            {
                ...
                float4 heatmap = OverlayHeapMapColor(cost, costScreenTotal, 0.4);
                return heatmap;
            }
            ENDHLSL
        }
