...
## [2.0.13] - 2024-07-04
### Fixed
- Fixed ShaderGraph injection null exception on Unity 6.
- Fixed null exception `InstancedTerrainFoliage` in when an `InstancedPrototype` is missing.
- Fixed possible out of range error in `InstancedTerrainFoliage` mark region dirty.
- Fixed `InstancedTerrainFoliage` not working with MicroVerse.
- Fixed `InstancedTerrainFoliage` bather dirty cells not being burst compiled.
### Changed
- Added support for patching MicroVerse and BetterShaders shaders.
...
- Fixed convert GameObjects to instances not checking for child `MeshRenderer` components.
### Changed
